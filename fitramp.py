...
def fit_ramps(
    ...
    1. diffs [resultant differences, shape (ndiffs, npix)]
            XXX - (ngroups, ncols) of current row
...
def get_ramps(countrate, sig, readtimes, nramps=10):

    """
    Function get_ramps: make some synthetic ramps
    Arguments:
    ...
    for k in range(len(readtimes)):
        ...
        if hasattr(t, "__len__"):
            ...
            # add read noise
            tmp = np.random.randn(nramps) * sig
            counts[k] += tmp
